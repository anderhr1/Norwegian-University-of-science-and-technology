...
\end{align}
where $i$ is stock and $\tau$ is the current period and $\Sigma$ is the total number of periods. We calculate the RMSE for each bucket and each period, summing the total RMSE for each bucket over the total sample period.

Further, we use the "long-short" metric to assess whether our forecasting model is able to predict the correct direction of the stock return. 