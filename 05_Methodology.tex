...
In the first part of the study we calculate the daily idiosyncratic volatility (IVOL) for each of the selected stocks by applying the Fama French regression.

Secondly, we pool the stocks into four equally sized, equally weighted portfolios by their constituents proportion of daily IVOL to daily volatility, which we denote as the daily IVOL percentage. The equally weighted portfolios are rebalanced daily.
...
\begin{figure}
    ...
    \captionof{figure}{Flow Chart of the methodology}
    \label{FlowChart}
\end{figure}
...
In this section, we describe how we implemented the Fama French three factor model, before we define IVOL.
...
Consistent with Ang et al. \cite{angetal06} we define IVOL as the square root of the variance of the error term in the Fama French three factor model \cite{famafrench}. Hence, deriving from equation \ref{FFregression}, we have:
 ...
 \end{align}
where $t$ is the current day, T is the number of days the rolling window runs over and $IV_{i,t}$ is the IVOL of stock $i$ in day $t$. Here we assume the ordinary OLS assumptions of the linear regression model. 

\section{Portfolio construction pooled by the constituents' daily IVOL percentage}
We have now described the methodology of estimating the daily IVOL percentage values. In this study, we seek to examine the relationship between the proportion of a stock's IVOL to volatility, the IVOL percentage. This because we find it interesting to examine stocks where the IVOL constitutes a large or small component of the stock's volatility, and not just in terms of magnitude. We define the daily IVOL percentage as the ratio:
\begin{align}
